...
export default function Header(){
    ...
    const [menuStyle , setMenuStyle ] = useState('bg-amber-900/30')
    ...
    const headerClick = () => {
        ...
        if (mobile ===  'hidden'){
            setMobile('block')
            setMenuStyle('bg-red-900 p-2 w-10 rounded-full')
            setMenuEventer('X')
        }
        else{
            setMobile('hidden')
    ...
    return (
        <>
        <div className={`${headerFixed ? "fixed  " : "relative"} bg-gradient-to-tl from-amber-800/40 to-purple-950/50 p-5 w-full   hidden md:block`} ref={headerRef}>
           <ul className='justify-center flex flex-row gap-10'>
            {headerContent.map((item) => ( 
                <li key= {item} ><button onClick={() => {sectionClick(item)}}>{item}</button></li>
            ))}
           </ul>
            
        </div>
        <div className={`${headerFixed ? "fixed  " : "relative"} top-0 fixed-0  block md:hidden  p-2`}>
